    ...
    <view class="confirm" v-if="!showPayPopup">
              ...
              <view class="specification-content">
                ...
                <view v-if="payInfo.isLogin && payInfo.pointAmount >= 0.01 && payInfo.canUsedAsMoney == 'true'" class="pay-item">
                    <view class="item-point">
                        <view class="title">
                           <text class="iconfont icon-success"></text>
                           <text v-if="payInfo.usePoint > 0" class="point-amount">使用{{ payInfo.usePoint }}积分抵扣</text>
        ...
        <view class="point-popup">
           <uni-popup ref="pointPopup" type="dialog">
              <uni-popup-dialog mode="input" focus="false" v-model="payInfo.usePoint" title="修改积分数量"  type="info" placeholder="请输入积分数量" :before-close="true" @close="closeDialog" @confirm="doUsePoint"></uni-popup-dialog>
           </uni-popup>
        </view>
        <view class="coupon-popup">
           <uni-popup ref="couponPopup" type="dialog">
              <uni-popup-dialog focus="false" v-if="payInfo.couponAmount > 0" v-model="payInfo.usePoint" title="确认信息" content="不使用卡券进行抵扣？" type="info" :before-close="true" @close="closeDialog" @confirm="doUseCoupon"></uni-popup-dialog>
